...
export default function Navbar({ cartCount = 0 }) {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    // In a full app this would trigger a search route or filter products
    // For now we just clear the input to show responsiveness
    setQuery("");
  };
  ...
  return (
    <header className="w-full sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/60 bg-white/80 border-b">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button className="relative inline-flex items-center justify-center h-10 w-10 rounded-lg border hover:bg-neutral-50">
